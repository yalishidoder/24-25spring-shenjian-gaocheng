...
void calc_next(char d_cur[], char d_next[])
{
	...
	for (i = 1; i <= len; i++) {
		d_next[i] = (rest + d_cur[i]) / 2;
		rest = (rest + d_cur[i]) % 2 * 10;
	}
	d_next[i] = (rest + d_cur[i]) / 2;
}
...
void calc_num(char pure_decimal[], char out[], char power_table[][N_NUM + 1])
{
	...
	for (i = len; i > 0; i--) {
		...
		for (int power = 1; power <= len; power++) 
			if (pure_decimal[power] == 1)
				sum += power_table[power][i];
		out[i] = sum % 10;
		rest = sum / 10;
	}
}
...
void input_pure_decimal(char pure_decimal[])
{
	...
	while (1) {
		int n_one = 0, n_d = 0, n_zero = 0, i = 0;
		for (i = 0; i < N_NUM; i++) {
			pure_decimal[i] = 0;
		}
		...
		else if (n_one + n_zero < n_d - 1) {
			cout << "输入的不是0/1，请重输" << endl;
			continue;
		}
		else if(n_zero == n_d - 1 && n_d >1) {
			cout << "输入为全0，请重输" << endl;
			continue;
		}
		else if(n_d == 1 && pure_decimal[0] == '.') {
