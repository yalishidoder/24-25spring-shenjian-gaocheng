...
struct tj_time *tj_time_convert(int input_time)
{
	...
	int tianshu[13] = { 0, 31,28,31,30,31,30,31,31,30,31,30,31 }; //包含每个月的天数
	...
	result.tj_minute -= ((result.tj_hour - 8) * 60);
	result.tj_hour -= ((result.tj_day - 1) * 24);
	...
	while (1) {
		if (result.tj_day > tianshu[result.tj_month]) {
			result.tj_day -= tianshu[result.tj_month];
			result.tj_month++;
